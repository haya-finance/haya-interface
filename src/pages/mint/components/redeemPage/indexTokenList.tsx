...
export default function RedeemIndexTokenList({ onClose, data, windowWidth }: Props) {
              ...
              <Box key={index} component="button" sx={{ cursor: 'pointer', width: '100%', backgroundColor: 'transparent', border: 0, padding: windowWidth >= 600 ? "16px 20px" : "12px 12px", "&:hover": { backgroundColor: '#f6f6f6', borderRadius: '20px' }, "&:focus": { backgroundColor: '#f6f6f6', borderRadius: '20px' } }} onClick={() => handleListClick(item.symbol)} >
                <Stack direction="row" alignItems="center" justifyContent="space-between" width="100%">
                  <Stack direction="row" alignItems="center" spacing="6px">
                    ...
                    <Stack spacing="2px">
                      <Typography variant="inherit" sx={{ color: '#000', fontSize: '13px', fontWeight: 500 }}>{item.symbol}</Typography>
                      <Typography variant="inherit" sx={{ color: '#6f6f6f', fontSize: '13px', fontWeight: 500 }}>{item.symbol}</Typography>
                    </Stack>
                  </Stack>
